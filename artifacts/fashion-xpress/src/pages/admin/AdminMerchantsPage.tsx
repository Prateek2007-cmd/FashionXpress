...
export function AdminMerchantsPage() {
  ...
  const [editState, setEditState] = useState<MerchantEditState>({ name: '', email: '', phone: '' });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: 'Deleted', description: 'Merchant removed.' });
      queryClient.invalidateQueries({ queryKey: ['/api/merchants'] });
      setDeletingId(null);
    },
    onError: (err: any) => {
  ...
  const startEdit = (merchant: any) => {
    setEditingId(merchant.id);
    setEditState({ name: merchant.name, email: merchant.email, phone: merchant.phone || '' });
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    createMerchant.mutate({ data: formData }, {
      onSuccess: () => {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto text-white">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-serif mb-2">Merchants</h1>
          <p className="text-muted-foreground">Manage your merchants and product uploaders.</p>
        </div>
        <Button onClick={() => setIsAdding(true)} className="gap-2">
          <UserPlus className="w-4 h-4" /> Add Merchant
        </Button>
      </div>

      {isAdding && (
        <div className="bg-card/30 border border-white/10 rounded-xl p-6 mb-8">
          <h2 className="text-xl font-serif mb-6 text-primary">Create New Merchant Account</h2>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-wider text-muted-foreground">Full Name</label>
              <Input required value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="bg-black/40 border-white/10" placeholder="e.g. Zara Boutique" />
            </div>
            <div className="space-y-2">
              <label className="text-xs uppercase tracking-wider text-muted-foreground">Email</label>
              <Input required type="email" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} className="bg-black/40 border-white/10" placeholder="zara@fashionxpress.com" />
            </div>
            <div className="space-y-2">
            ...
            <div className="md:col-span-2 flex justify-end gap-3 mt-4">
              ...
              <Button type="submit" disabled={createMerchant.isPending}>
                {createMerchant.isPending && <Loader2 className="w-4 h-4 animate-spin mr-2" />}
                Create Account
              </Button>
            </div>
          </form>
      ...
      <div className="bg-card/30 border border-white/10 rounded-xl overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-muted-foreground">No merchants found. Create one to get started.</div>
        ) : (
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-white/5">
                {merchants?.map((merchant) => {
                  ...
                  const isDeleting = deletingId === merchant.id;

                  return (
                    <tr key={merchant.id} className={`transition-colors ${isEditing ? 'bg-primary/5' : 'hover:bg-white/5'}`}>
                      <td className="p-4">
                        {isEditing ? (
                          <Input
                            value={editState.name}
                            onChange={e => setEditState({ ...editState, name: e.target.value })}
                            className="bg-black/40 border-white/10 h-9 text-sm w-48"
                            placeholder="Name"
                          />
                        ) : (
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-primary/20 overflow-hidden flex items-center justify-center border border-primary/30 shrink-0">
                              <span className="text-primary font-semibold text-lg">{merchant.name.charAt(0)}</span>
                            </div>
                      ...
                      <td className="p-4 text-muted-foreground">
                        {isEditing ? (
                          <div className="space-y-1.5">
                            <Input value={editState.email} onChange={e => setEditState({ ...editState, email: e.target.value })} className="bg-black/40 border-white/10 h-9 text-sm w-56" placeholder="Email" />
                            <Input value={editState.phone} onChange={e => setEditState({ ...editState, phone: e.target.value })} className="bg-black/40 border-white/10 h-9 text-sm w-56" placeholder="Phone" />
                          </div>
                        ) : (
                          <>
                            <div className="mb-1">{merchant.email}</div>
                          ...
                          {isEditing ? (
                            <>
                              <Button
                                ...
                                onClick={() => updateMutation.mutate({ id: merchant.id, data: editState })}
                            ...
                            <>
                              ...
                              <Button
                                ...
                                onClick={() => setDeletingId(merchant.id)}
                              >
