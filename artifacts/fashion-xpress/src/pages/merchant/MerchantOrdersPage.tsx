...
export function MerchantOrdersPage() {
  ...
  const fetchOrders = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`${API_BASE}/api/orders/merchant`, {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="font-serif text-3xl text-white mb-1">My Orders</h1>
          <p className="text-muted-foreground text-sm tracking-widest uppercase">
            Manage customer reservations & orders for your dresses
          </p>
        </div>
        <button
          onClick={fetchOrders}
      ...
      {orders.length === 0 ? (
        <div className="text-center py-20 border border-white/5 rounded-xl bg-card/20">
          ...
          <p className="text-muted-foreground mb-2">No orders received yet.</p>
          <p className="text-muted-foreground text-sm">When customers reserve your dresses, they will show up here.</p>
        </div>
      ) : (
        <div className="space-y-6">
          {orders.map((order, idx) => {
            const orderTotal = order.items.reduce(
              (sum, item) => sum + parseFloat(item.priceAtSale) * item.quantity, 
              0
            );
            ...
            return (
              ...
              >
                ...
                <div className="px-6 py-4 bg-white/[0.02] border-b border-white/5 flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      <span className="text-xs text-muted-foreground uppercase tracking-wider block">Your Items Total</span>
                      <span className="font-medium text-primary text-lg">{formatPrice(orderTotal)}</span>
                    </div>
                    <div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x divide-white/5">
                  {/* Left/Center Columns: Product Details */}
